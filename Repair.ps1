...
param([ValidateSet('Diagnose','ResetClientCache','FlushDns')][string]$Action='Diagnose',[string]$OutputPath=(Join-Path ([Environment]::GetFolderPath('Desktop')) 'Microsoft_Lists_Repair'))
...
[ordered]@{Action=$Action;Processes=@(Get-Process msedge,'ms-teams' -ErrorAction SilentlyContinue|Select-Object Name,Id);Caches=@($cachePaths|ForEach-Object{[pscustomobject]@{Path=$_;Exists=Test-Path $_}});Lists443=(Test-NetConnection 'lists.microsoft.com' -Port 443 -InformationLevel Quiet -WarningAction SilentlyContinue);Graph443=(Test-NetConnection 'graph.microsoft.com' -Port 443 -InformationLevel Quiet -WarningAction SilentlyContinue)}|ConvertTo-Json -Depth 5|Set-Content (Join-Path $OutputPath "PreRepair_$stamp.json")
